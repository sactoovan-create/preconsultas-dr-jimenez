...
function renderBloque(b) {
  ...
  if (b.tipo === 'conclusion') {
    const tono = TONO[b.tono] ? b.tono : 'neutro';
    return `<div class="hc-b">${enc}<div class="hc-concl hc-t-${tono}"><div class="hc-titular">${escaparHtml(b.titular || '')}</div>${b.detalle ? `<div class="hc-det">${escaparHtml(b.detalle)}</div>` : ''}</div></div>`;
  }
  if (b.tipo === 'tabla') {
...
function contextoHtml(p) {
  ...
  const ident = [
    `<div class="ident-nombre"><b>Paciente</b><span class="v">${escaparHtml(dem.nombre || 'Sin identificar')}</span></div>`,
  ...
  const ant = marcados.length
    ? `<div class="hc-ant"><span class="hc-ant-et">Antecedentes / contraindicaciones</span>${marcados.map((k) => escaparHtml(ANTECEDENTES[k])).join(' · ')}</div>`
    : '';
...
<style>
  :root{--verde:#1F3A2E;--verde-tinta:#16291F;--dorado:#A88B5C;--oro:#6D5730;--terracota:#B85042;--ambar:#C16A3A;--ok:#3A6B4C;--crema:#F6F1E6;--linea:rgba(31,58,46,.14);--hair:rgba(168,139,92,.55);--tinta:#233A2E;}
  *{box-sizing:border-box;margin:0;padding:0;}
  body{font-family:'Inter',system-ui,sans-serif;color:var(--tinta);padding:0;line-height:1.5;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
  ...
  .marca-wm{font-family:'Inter',sans-serif;font-weight:600;font-size:27px;letter-spacing:-.03em;color:var(--verde);line-height:1;display:flex;align-items:center;}
  .marca-wm .pt{width:7px;height:7px;border-radius:50%;background:var(--dorado);margin:0 3px 2px 5px;align-self:flex-end;display:inline-block;}
  .marca-desc{font-size:9.5px;letter-spacing:.3em;text-transform:uppercase;color:var(--oro);font-weight:500;margin-top:8px;}
  .cab-der{text-align:right;font-size:9.5px;line-height:1.7;color:rgba(31,58,46,.66);}
  .cab-der .nom{font-weight:700;font-size:11px;color:var(--verde);letter-spacing:.01em;}
  .cab-der .ced{color:var(--oro);}

  /* Identidad de la paciente */
  .ident{display:flex;gap:34px;flex-wrap:wrap;margin:16px 0 0;padding:12px 16px;background:var(--crema);border-radius:10px;}
  .ident b{display:block;font-size:8.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:var(--oro);margin-bottom:2px;}
  .ident .v{font-size:14px;font-weight:600;color:var(--verde);}
  .ident-nombre .v{font-family:'Fraunces',serif;font-weight:400;font-size:20px;}

  .hc-ant{font-size:11.5px;color:var(--terracota);font-weight:600;background:rgba(184,80,66,.08);border:1px solid rgba(184,80,66,.3);border-radius:9px;padding:10px 14px;margin-top:10px;}
  .hc-ant-et{display:block;font-size:8.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:var(--terracota);margin-bottom:3px;opacity:.85;}

  .eyebrow{display:inline-flex;align-items:center;gap:8px;font-size:9.5px;font-weight:700;letter-spacing:.22em;text-transform:uppercase;color:var(--oro);margin:20px 0 6px;}
  .eyebrow::before{content:'';width:5px;height:5px;border-radius:50%;background:var(--dorado);}
  .titulo{font-family:"Fraunces",serif;font-size:29px;font-weight:300;letter-spacing:-.02em;line-height:1.05;margin-bottom:4px;color:var(--verde);}
  .nota{font-size:10px;color:rgba(31,58,46,.5);margin-bottom:4px;}

  .hc-b{margin-top:15px;break-inside:avoid;}
  .hc-enc{font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--verde);padding-bottom:6px;margin-bottom:9px;position:relative;}
  ...
  /* Conclusiones como tarjetas de tono: la prioridad se lee de un vistazo */
  .hc-concl{padding:11px 14px;border-radius:9px;border-left:4px solid var(--linea);}
  .hc-titular{font-family:"Fraunces",serif;font-size:19px;font-weight:400;line-height:1.16;color:var(--verde);}
  .hc-det{font-size:11.5px;color:rgba(31,58,46,.75);margin-top:5px;line-height:1.5;}
  .hc-t-neutro{background:rgba(31,58,46,.03);border-left-color:rgba(31,58,46,.35);}
  .hc-t-ok{background:rgba(58,107,76,.08);border-left-color:var(--ok);}
  .hc-t-ok .hc-titular{color:var(--ok);}
  .hc-t-aviso{background:rgba(193,106,58,.09);border-left-color:var(--ambar);}
  .hc-t-aviso .hc-titular{color:#9a5227;}
  .hc-t-alerta{background:rgba(184,80,66,.1);border-left-color:var(--terracota);}
  .hc-t-alerta .hc-titular{color:var(--terracota);}

  .hc-t{width:100%;border-collapse:collapse;font-size:12.5px;}
  .hc-t th{text-align:left;font-size:9px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--oro);padding:0 6px 6px;border-bottom:1px solid var(--linea);}
  .hc-t td{padding:8px 6px;border-bottom:1px solid var(--linea);}
  .hc-t tr:nth-child(even) td{background:rgba(31,58,46,.02);}
  .hc-t .par{font-weight:600;color:var(--verde);}
  .hc-l{display:flex;justify-content:space-between;gap:16px;padding:7px 0;border-bottom:1px solid var(--linea);font-size:12.5px;}
  .hc-le{color:rgba(31,58,46,.6);} .hc-lv{font-weight:600;text-align:right;color:var(--verde);}
  .hc-ul{font-size:12px;line-height:1.6;padding-left:18px;} .hc-ul li{margin-bottom:5px;}
  .hc-p{font-size:12px;line-height:1.6;color:rgba(31,58,46,.85);}
  .hc-flags{margin-top:16px;background:rgba(184,80,66,.09);border:1px solid rgba(184,80,66,.35);border-left-width:4px;border-radius:9px;padding:12px 14px;break-inside:avoid;}
  ...
  .hc-flags ul{padding-left:16px;font-size:12px;line-height:1.55;color:#7d2f24;} .hc-flags li{margin-bottom:4px;}

  .pie{background:var(--crema);padding:12px 16px;border-radius:9px;margin-top:24px;border-top:1px solid var(--linea);font-size:9.5px;color:rgba(31,58,46,.55);line-height:1.5;break-inside:avoid;}
  .credito{margin-top:9px;text-align:center;font-size:8.5px;letter-spacing:.08em;color:var(--oro);}
...
<body>
  <div class="cab">
    ...
    <div class="cab-der">
      ...
      <div>drivanjimenezginecologo.com</div>
    </div>
  </div>
